...
const create = () => users.save('user.test@test.com', '123456')
...
test('Login de usuário - sucesso', async t => {
  await create()
  const result = await auth.authenticate('user.test@test.com', '123456')
...
test('Login de usuário - falha', async t => {
  await create()
  const promise = auth.authenticate('usertest@test.com', '123456')
